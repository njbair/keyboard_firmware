...
enum function_id {
    ...
    F_KP_SWITCH_LAYOUT_NUMPAD,
    F_KP_SWITCH_LAYOUT_GAMER,
};
...
void action_function(keyrecord_t *record, uint8_t id, uint8_t opt __attribute__ ((unused))) {
    switch (id) {
        ...
        case F_SWITCH_LAYOUT_GAMER:
            ...
            break;
        /**
         * Switch the Monarch keypad layout
         */
        case F_KP_SWITCH_LAYOUT_NUMPAD:
            if (record->event.pressed) {
                layer_off(L_KP_GAMER);
                clear_keyboard();
            }
            break;
        case F_KP_SWITCH_LAYOUT_GAMER:
            if (record->event.pressed) {
                layer_on(L_KP_GAMER);
                clear_keyboard();
            }
